...
var useLevelgraph = false; //possibilite d'utiliser LevelGRAPH DB : opérationnel (stocké dans daossier data)
var useFuseki = false; //possibilité d'utiliser un serveur fuseki sur http://127.0.0.1:3000
var noBase = true;

if (noBase){
//  stockage en mémoire des donnees et sauvegarde sur fichier car leveldown ne s'install pas sur Arduino Yun avec nodejs0.10.25
var memData = {messages:[], nodes:[], edges:[]};
var fileData = {};
}
...
io.on('connection', function (socket) {
  ...
  socket.on('new message', function (data) {
    ...
    socket.broadcast.emit('new message', mess);
    if (noBase){
      memData.messages.push(mess);
      console.log(memData);
    }
  });
  ...
  socket.on('add user', function (username) {
    ...
    if (noBase){
      ...
      console.log(memData);
      var list = memData.nodes.concat(memData.edges);
  ...
  socket.on('newActions', function(actions) {
    ...
    actions.forEach(function(action) {
      ...
      switch (action.type) {
        ...
        console.log(triple);
          if (noBase){
            memData.nodes.push(triple);
            console.log(memData);
          }else if (useLevelgraph){
        db.get({
          subject: data.id,
          predicate: "label",
          type: "node"
          ...
          if (list.length == 0) {
            ...
            db.put(triple, function(err, putted) {
              console.log("added");
            });
          }
        });}
        ...
        if (noBase){
        console.log("DELETE NODE NOBASE");
          console.log(memData);
        }else if (useLevelgraph){
        db.get({
          subject: nodeId
      ...
      //  var triple = [tripleLabel, tripleFrom, tripleTo];
        memData.edges.push(tripleLabel);
        memData.edges.push(tripleFrom);
        memData.edges.push(tripleTo);
          console.log(memData);
        }else if (useLevelgraph){
        if (edge != undefined) {
          db.get({
